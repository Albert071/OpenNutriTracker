...
- [The X-Catalog-Access header](#the-x-catalog-access-header)
...
Before the wizard opens, the app runs a tight reachability probe against the CDN so a user with no connection (or an outage on our side) sees a clear message rather than walking into a wizard that will fail at its first network hop. The gate sits in two places, working together: a mount-time probe in the bloc that surfaces the tile subtitle for fresh installs, and a tap-time probe at the tile that gates wizard entry universally.

The mount-time probe runs when `LoadCatalogStatusEvent` resolves that there is no catalog on disk and no partial download to resume. The bloc emits `checking` and calls `CheckCatalogAvailabilityUseCase`. That use case delegates to `CatalogDownloadDataSource.probeAvailability`, which issues a HEAD against the recommended default variant's manifest URL with a five-second timeout and folds every failure mode (timeout, DNS error, TLS error, non-2xx response) into a simple `false`. The recommended default variant `s1_n1_r5` is always present when the catalog pipeline is healthy, so reaching it successfully proves the whole chain (DNS, edge, R2, cache rule) is working end to end. If the probe returns `true` the bloc emits `idle` and the wizard is unlocked the way it always was; if it returns `false` the bloc emits `unavailable` and the tile shows the "try again later" message in the user's locale.

The tap-time probe is the gate that actually keeps the wizard from opening. It runs on every tap, regardless of what the bloc state currently says, because the bloc is a lazy singleton and its cached `idle` or `ready` from a previous session is what the tile renders for the very first frame after a re-mount. A fast tap in that brief window would otherwise fall through to the wizard before the new mount-time probe had finished. The tap-time probe also gates entry for users who already have a catalog on disk: the bloc skips the mount-time probe in that case so search keeps working offline, but the wizard's primary value (download or refresh) still needs the CDN. The tap-time probe is the single source of truth for whether the wizard should open. While it is in flight the tile shows "Checking availability…" and disables further taps; if it fails the tile flips to "Caching currently unavailable, try again later" and a snackbar acknowledges the rejected tap. The flag clears at the start of the next probe so a retry shows "Checking" again before settling on the new result.

The mount-time probe is deliberately not run when a catalog is already on disk or a paused download exists. A user with installed data can search it happily without ever talking to the CDN; gating the tile subtitle on availability in that case would block them from a feature they have already paid for. The tap-time probe still runs and gates wizard entry, so the user can search but not open the wizard until the bucket is reachable again.

There is also a defence-in-depth pop in the wizard itself. If an availability transition somehow happens while the wizard is open (a user who entered before the first probe completed, a probe that flipped during a long-running session), the wizard's `BlocListener` shows a snackbar and pops back to settings rather than letting the user continue into a flow that is going to fail. The tap-time probe is the primary gate; the wizard pop is the safety net.

## The X-Catalog-Access header

Every catalog HTTP request — the HEAD probe, the manifest GET, every chunk GET, the streamed chunk send — carries an `X-Catalog-Access` header with a shared bearer token. A Cloudflare WAF Custom Rule on the `opennutritracker.org` zone (`cloudflare_ruleset.catalog_access` in `opentofu/infrastructure/modules/cloudflare/firewall.tf`) blocks any request to `catalog.opennutritracker.org` whose header does not match the expected value, returning a 403 at the edge before R2 is ever touched. The gate exists so a random crawler with the URL cannot pull a 500 MiB sqlite of Open Food Facts data without going through the app — the security ceiling is set by APK reverse-engineering effort, not by the cipher (there isn't one), but the friction is enough to keep the bucket out of casual abuse, and a blocked request consumes zero R2 reads.

The token itself comes from the `CATALOG_ACCESS_TOKEN` env var, obfuscated at compile time by `envied` and surfaced as `Env.catalogAccessToken`. The value lives in three places: in the OpenTofu state (where it's generated by `random_password.catalog_access_token` and held in the encrypted state file), as a sealed `CATALOG_ACCESS_TOKEN` GitHub Actions secret (for the eventual automated APK release pipeline), and in the gitignored local `.env` (for today's manual `fvm flutter build apk` workflow). The OpenTofu reference in [`docs/opentofu.md`](opentofu.md) is the source of truth for retrieving the value and for the emergency-rotation procedure.

The header is added once, at client-construction time, via `ONTHttpClient`'s `extraHeaders` parameter. The data source declares the headers map in one place (`_catalogRequestHeaders`) and passes it to every `ONTHttpClient` it constructs — the probe, the manifest fetch, every worker. There is no per-request header plumbing, which means there is no failure mode where a new endpoint forgets to attach the token.

A 403 from the gate is surfaced as `CatalogAccessDeniedException`. The bloc renders this as a non-recoverable error and the wizard tells the user to update OpenNutriTracker — the only practical way out, because a rotated token means the build sitting on the user's phone pre-dates the rotation and retrying will keep getting 403s.
